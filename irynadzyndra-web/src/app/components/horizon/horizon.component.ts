...
export class HorizonComponent implements OnInit {
  ...
  constructor(public pageData: PageDataService) {
    this.images = [];
    for (let i = 0; i < this.pageData.horizonCount; ++i) {
      this.images.push({
        url: this.pageData.horizonImageUrlArray[i],
        title: this.pageData.allData.imagesHorizonNotes[i],
        thumbnailUrl: this.pageData.horizonImageUrlArray[i],
